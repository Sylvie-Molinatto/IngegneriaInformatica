...
public class SparkDriver {
	...
	public static void main(String[] args) {
		...
		JavaRDD<String> labeledData = data.filter( line -> {
			String[] fields = line.split(",(?=([^\"]*\"[^\"]*\")*[^\"]*$)");
			if (fields[0].equals("Id"))
				return false;
			else if (Integer.parseInt(fields[5])==0)
				return false;
			else
				return true;
		}); 
    	...
    	//EX 2: CREATE THE PIPELINE THAT IS USED TO BUILD THE CLASSIFICATION MODEL (with decision tree)
        Pipeline pipeline = new Pipeline()
				.setStages(new PipelineStage[] {new DecisionTreeClassifier().setLabelCol("label").setFeaturesCol("features")});
