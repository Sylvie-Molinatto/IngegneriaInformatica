    ...
    <ListGroup as="ul" variant="flush">
      {
        filterArray.map(([filterName, { label}]) => {
          return (
            <NavLink className="list-group-item" key={filterName} to={`/filter/${filterName}`} style={{ textDecoration: 'none' }}>
              <ListGroup.Item as="li" key={filterName} action active={selected === filterName} >
                {label}
              </ListGroup.Item>
            </NavLink>
          );
